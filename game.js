...
const Game = (() => {
  ...
  // sky above the horizon + a perspective floor grid below it (depth lines
  // converging toward the horizon, lane lines converging toward the front)
  function drawFloor3D(W, H, geo){
    const { horizonY, edgeX, rowY, laneT } = geo;

    // sky: dark gradient with a soft glow sitting right on the horizon
    const sky = fieldCtx.createLinearGradient(0, 0, 0, horizonY);
    sky.addColorStop(0, '#0a0e14');
    sky.addColorStop(1, '#1a2432');
    fieldCtx.fillStyle = sky;
    fieldCtx.fillRect(0, 0, W, horizonY);

    const glow = fieldCtx.createRadialGradient(W/2, horizonY, 0, W/2, horizonY, W*0.55);
    glow.addColorStop(0, 'rgba(217,166,62,0.16)');
    ...
    floor.addColorStop(0, '#141c26');
    floor.addColorStop(1, '#0d1219');
  ...
  function drawUnit(x, y, u, isMine, engageX){
    ...
    const bob = Math.sin(performance.now() / 260 + u.id) * 1.4;
